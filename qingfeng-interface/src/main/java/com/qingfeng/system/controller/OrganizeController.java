...
public class OrganizeController extends BaseController {
    ...
    public void findList(@RequestHeader HttpHeaders headers,@RequestBody PageData pd, HttpServletResponse response) throws IOException  {
		...
		pd.put("auth_organize",organize_id);
		pd.put("user_id",user_id);
		PageData organize = userService.findUserOrganizeInfo(pd);
		pd.put("id",user_id);
	...
	public void del(@RequestHeader HttpHeaders headers, @RequestParam String ids, HttpServletResponse response) throws IOException  {
		...
		pd.put("organize_ids",Arrays.asList(del_ids));
		//删除用户组织信息
		userService.delUserOrganize(pd);
		//删除组织角色信息
		organizeService.delOrganizeRole(pd);
		organizeService.del(del_ids);
		Json json = new Json();
		json.setSuccess(true);
		json.setMsg("操作成功。");
		this.writeJson(response,json);
	...
	public void updateAuth(@RequestHeader HttpHeaders headers, @RequestBody PageData pd,HttpServletResponse response) throws IOException  {
		Json json = new Json();
		String time = DateTimeUtil.getDateTimeStr();
